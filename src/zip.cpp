...
void changeKeys(std::istream& is, std::ostream& os, const Keys& oldKeys, const Keys& newKeys, std::atomic<Progress>* progress)
{
    ...
    int done = 0;
    if(progress)
        *progress = {done, static_cast<int>(packedSizeByLocalOffset.size())};
